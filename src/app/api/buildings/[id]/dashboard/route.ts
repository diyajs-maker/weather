...
export async function GET(
  ...
  try {
    ...
    const latestEnergyReport = energyReportResult.length > 0 ? {
      month: (energyReportResult[0] as any).month,
      year: (energyReportResult[0] as any).year,
      savingsPercentage: Number((energyReportResult[0] as any).savings_percentage),
      savingsKBTU: Number((energyReportResult[0] as any).savings_kbtu),
      pdfUrl: (energyReportResult[0] as any).pdf_url,
    } : null;
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        totalAlerts: parseInt((alertsResult[0] as any).total_alerts),
        totalRecipients: parseInt((recipientsResult[0] as any).total_recipients),
        days,
      },
      ...
      recentUploads: recentUploadsResult.map((upload: any) => ({
        id: upload.id,
