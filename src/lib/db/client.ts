...
export const db = {
  async query(text: string, params?: any[]) {
    try {
      ...
      return { rows: result }; // Return in a format compatible with existing code if possible, or update all callers
    } catch (error) {
  ...
  async getCityById(id: string): Promise<City | null> {
    ...
    `;
    return result[0] ? (result[0] as City) : null;
  },
  ...
  async saveTemperatureSnapshot(data: Omit<TemperatureSnapshot, 'id' | 'created_at'>): Promise<TemperatureSnapshot> {
    const result = await sql`
      ...
      RETURNING *
    `;
    return result[0] as TemperatureSnapshot;
  },
