...
export class EnergyService {
  async uploadUtilityData(
    ...
    return result[0] as UtilityConsumption;
  }
  ...
  async calculateBaseline(
    ...
    if (utilityData.length < 3) {
      return { heating: null, cooling: null };
    }

    const cityId = (utilityData[0] as any).city_id;
    ...
    const hddMap = new Map(degreeDaysData.map(dd => [`${(dd as any).year}-${(dd as any).month}`, (dd as any).heating_degree_days]));
    const cddMap = new Map(degreeDaysData.map(dd => [`${(dd as any).year}-${(dd as any).month}`, (dd as any).cooling_degree_days]));
    ...
    for (const utility of utilityData) {
      const u = utility as any;
      const key = `${u.year}-${u.month}`;
      const hdd = hddMap.get(key);
      const cdd = cddMap.get(key);

      if (hdd && hdd > 0) {
        heatingSum += Number(u.total_kbtu) / Number(hdd);
        heatingCount++;
      }

      if (cdd && cdd > 0) {
        coolingSum += Number(u.total_kbtu) / Number(cdd);
        coolingCount++;
      }

      if (u.year < minYear) minYear = u.year;
      if (u.year > maxYear) maxYear = u.year;
    }
  ...
  async saveBaseline(
    ...
    return result[0] as EnergyBaseline;
  }
  ...
  async calculateMonthlyComparison(
    ...
    const hdd = Number((degreeDays[0] as any).heating_degree_days);
    const cdd = Number((degreeDays[0] as any).cooling_degree_days);
    const totalKBTU = Number((utility[0] as any).total_kbtu);
    ...
    const baselineHDD = heatingBaseline.length > 0
      ? Number((heatingBaseline[0] as any).avg_consumption_per_degree_day)
      : null;
    const baselineCDD = coolingBaseline.length > 0
      ? Number((coolingBaseline[0] as any).avg_consumption_per_degree_day)
      : null;
    ...
    return {
      ...
      electricKWH: (utility[0] as any).electric_kwh ? Number((utility[0] as any).electric_kwh) : null,
      gasTherms: (utility[0] as any).gas_therms ? Number((utility[0] as any).gas_therms) : null,
      fuelOilGallons: (utility[0] as any).fuel_oil_gallons ? Number((utility[0] as any).fuel_oil_gallons) : null,
      districtSteamMBTU: (utility[0] as any).district_steam_mbtu ? Number((utility[0] as any).district_steam_mbtu) : null,
      totalKBTU,
      hdd,
      cdd,
    };
  ...
  async getBuildingUtilityHistory(
    ...
    return result as UtilityConsumption[];
  }
