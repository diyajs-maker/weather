...
export class ReportService {
  async generateMonthlyReport(
    ...
    return {
      ...
      comparison,
      baselineInfo: {
        heating: heatingBaseline.length > 0 ? {
          periodStart: (heatingBaseline[0] as any).baseline_period_start,
          periodEnd: (heatingBaseline[0] as any).baseline_period_end,
          dataPoints: (heatingBaseline[0] as any).data_points,
        } : undefined,
        cooling: coolingBaseline.length > 0 ? {
          periodStart: (coolingBaseline[0] as any).baseline_period_start,
          periodEnd: (coolingBaseline[0] as any).baseline_period_end,
          dataPoints: (coolingBaseline[0] as any).data_points,
        } : undefined,
      },
    };
  }
  ...
  async saveReport(
    ...
    if (existing.length > 0) {
      ...
      await sql`
        ...
        SET utility_consumption_id = ${utilityResult.length > 0 ? (utilityResult[0] as any).id : null},
            degree_days_id = ${degreeDaysResult.length > 0 ? (degreeDaysResult[0] as any).id : null},
    ...
    const result = await sql`
      INSERT INTO energy_reports (
        ...
        ${utilityResult.length > 0 ? (utilityResult[0] as any).id : null},
        ${degreeDaysResult.length > 0 ? (degreeDaysResult[0] as any).id : null},
