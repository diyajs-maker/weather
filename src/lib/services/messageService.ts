  ...
  async sendPendingMessages(): Promise<{
    ...
    for (const msg of messages) {
      try {
        ...
        if ((msg as any).channel === 'sms') {
          const recipientResult = await sql`
            SELECT * FROM recipients WHERE id = ${(msg as any).recipient_id}
          `;
          const recipient = recipientResult[0];
          
          if ((recipient as any)?.phone) {
            const smsResult = await sendSMS((recipient as any).phone, (msg as any).content);
            success = smsResult.success;
            deliveryStatus = success ? 'delivered' : 'failed';
            error = smsResult.error;
          }
        } else if ((msg as any).channel === 'email') {
          const recipientResult = await sql`
            SELECT * FROM recipients WHERE id = ${(msg as any).recipient_id}
          `;
          const recipient = recipientResult[0];
          ...
          if ((recipient as any)?.email) {
            const subject = (msg as any).message_type === 'alert' 
              ? 'Temperature Alert - Action Required'
              : (msg as any).message_type === 'warning'
              ? 'Compliance Warning'
        ...
        console.error(`Error processing message ${(msg as any).id}:`, error);
        await sql`
          UPDATE messages SET delivery_status = ${'error'} WHERE id = ${(msg as any).id}
        `;
        failed++;
      }
    }
  ...
  async createMessagesFromAlert(alertLogId: string, cityId: string): Promise<string[]> {
    ...
    for (const building of activeBuildings) {
      ...
      for (const recipient of recipients) {
        ...
        const messageType = (alert as any).alert_type === 'sudden_fluctuation' 
          ? 'alert' 
          : 'daily_summary';

        const tempData = (alert as any).temperature_data;
